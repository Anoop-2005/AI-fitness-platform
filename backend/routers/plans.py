...
def weekly_review(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    with db.cursor() as cur:
        ...
        saved_row = cur.fetchone()
         ...
         # If ON CONFLICT DO NOTHING fired (another request beat us to it),
    # saved_row is None here — fetch the one that already exists.
        if saved_row is None:
            cur.execute("""
                SELECT id, user_id, week_start, stats, plateau_detected, summary, created_at
                FROM weekly_reviews
                WHERE user_id = %s AND week_start = %s
            """, (uid, week_start))
            saved_row = cur.fetchone()
