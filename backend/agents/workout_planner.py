"""
Builds a workout plan using LangGraph. The graph has three nodes, run once
per training day:

  fetch_candidates -> llm_select -> validate

  1. fetch_candidates: pure SQL/Python — decides which exercises are SAFE
     and AVAILABLE for this user (muscle group, equipment, injury flags).
  2. llm_select: the LLM picks and sequences exercises FROM that candidate
     list only.
  3. validate: every exercise the LLM returned is checked against the
     candidate list — anything it invented is dropped, never trusted.

This is "tool-calling discipline" made explicit as graph nodes, so the
LLM's job is strictly narrower than the plan-generation job as a whole.

The db connection, equipment list, and injury flags are request-scoped
(different per call), not part of the plan state that flows through the
graph — so the graph is built fresh per request via a factory function,
with those values captured as closures. This is the standard, version-safe
way to give LangGraph nodes access to things outside the persisted state.
"""
'''import json
...
def get_candidate_exercises(db, muscle_groups: list[str], equipment_available: list[str] | None,
    ...
    """Pure SQL filtering — no LLM involved. equipment_available=None means "any equipment is fine"."""
    ...
    if equipment_available is not None:
        ...
        rows = [r for r in rows if not r["equipment"] or all((e or "").lower() in allowed for e in r["equipment"])]
    return rows


def _parse_and_validate(llm_response: str, valid_ids: set[int], fallback: list[dict], sets_base: int) -> list[dict]:
    """Drops anything the LLM invented; falls back to a real, safe pick if parsing fails entirely."""
    try:
        ...
        pass
    return [{"wger_id": c["wger_id"], "sets": sets_base, "reps": "8-10", "rest_seconds": 75} for c in fallback[:3]]


def build_day_plan_graph(db, equipment_available: list[str] | None, injury_flags: list[str]):
    """
    Builds and compiles the 3-node LangGraph for one request's worth of
    training days. db/equipment_available/injury_flags are captured as
    closures — every day's graph.invoke() call reuses the same compiled
    graph, only `state` (muscle groups, sets_base, etc.) changes per day.
    """
    ...
    def llm_select_node(state: DayPlanState) -> dict:
        ...
        prompt = (
            ...
            f"{state['goal']}. Choose 2-4 exercises from this list ONLY (use the exact id numbers):\n"
...
def generate_workout_plan(db, profile: dict, review: dict | None) -> list[dict]:
    """
    Returns rows ready to insert into workout_plan_exercises:
    [{wger_id, day_number, sets, reps, rest_seconds, order_in_day}, ...]
    """
    days_per_week = max(1, min(profile["days_per_week"], 6))
    ...
    # Adaptive adjustment: plateau or low completion -> ease off; strong
    # adherence -> allow slightly more volume. This is the concrete
    # "adaptive" step — see DESIGN.md section 4a.
    ...
    for day_number, muscle_groups in enumerate(split, start=1):
        result = compiled_graph.invoke({
            "day_number": day_number, "muscle_groups": muscle_groups,
            "experience_level": profile["experience_level"], "goal": profile["primary_goal"],
            "sets_base": sets_base,
        })
        for order, item in enumerate(result["chosen"]):
            plan_rows.append({
                "wger_id": item["wger_id"], "day_number": day_number,
                "sets": item.get("sets", sets_base), "reps": item.get("reps", "8-10"),
                "rest_seconds": item.get("rest_seconds", 75), "order_in_day": order,
            })
    return plan_rows
'''
...
"""
Builds a workout plan using LangGraph. The graph has three nodes, run once
per training day with fresh state handling.
"""
import json
from typing import TypedDict
