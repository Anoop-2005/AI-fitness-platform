"""
Same LangGraph pattern as workout_planner.py: fetch_candidates -> llm_select
-> validate, applied to foods_cache instead of exercises_cache.
"""
'''import json
...
def get_candidate_foods(db, exclude_terms: list[str], limit: int = 40) -> list[dict]:
    query = "SELECT fdc_id, name, calories, protein_g, carbs_g, fat_g, fiber_g FROM foods_cache"
    params = []
    if exclude_terms:
        query += " WHERE NOT (" + " OR ".join(["name ILIKE %s"] * len(exclude_terms)) + ")"
...
def build_diet_plan_graph(db, exclude_terms: list[str]):
    ...
    def llm_select_node(state: DietPlanState) -> dict:
        ...
        prompt = (
            f"Pick one food for each of these meal slots: {', '.join(state['slots'])}. "
            ...
            f'Respond with ONLY a JSON array like [{{"fdc_id": "171077", "meal_slot": "breakfast"}}] — no other text.'
        )
...
def generate_diet_plan(db, profile: dict, analysis: dict, review: dict | None) -> list[dict]:
    """Returns rows ready for diet_plan_meals: [{fdc_id, day_number, meal_slot, order_in_day}, ...]"""
    meals_per_day = profile.get("meals_per_day", 5)
