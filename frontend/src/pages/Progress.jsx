...
export default function Progress() {
  ...
  // Aggregate logs into weekly or monthly buckets 
  ...
  // Keep chart colors aligned with the design system 
  const chartColors = { primary: "#2f6f4f", primaryDark: "#234f39", warning: "#b45309", danger: "#b3261e", purple: "#7c3aed", cyan: "#0891b2", grid: "#e5e7eb", text: "#6b7280", }; 
  const tooltipStyle = { backgroundColor: "#ffffff", borderColor: "#e5e7eb", borderRadius: "6px", color: "#111827", boxShadow: "0 2px 8px rgba(16, 24, 40, 0.08)", };
  ...
  return (
    <div className="page page-wide">
      ...
      <div className="flex-warp gap-8">
        ...
        onClick={() => setTimeRange("weekly")} > Weekly </button>

        <button 
        className={`btn ${ timeRange === "monthly" ? "" : "btn-secondary" }`} 
        onClick={() => setTimeRange("monthly")} > Monthly </button>
            ...
            <div className="chart-container"> 
              <ResponsiveContainer width="100%" height="100%"> 
                <LineChart data={formattedLogs.filter( (l) => l.bmi !== null )} > 
                  ...
                  <YAxis domain={["auto", "auto"]} fontSize={12} stroke={chartColors.text} /> 
                  <Tooltip contentStyle={tooltipStyle} /> 
                  <Line type="monotone" dataKey="bmi" stroke={chartColors.warning} strokeWidth={2} dot={{ r: 3 }} name="BMI" /> 
                </LineChart> 
              </ResponsiveContainer> 
            </div> 
          </div>
            ...
            <div className="chart-container">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={formattedLogs.filter(l => l.weight !== null)}>
                  ...
                  <YAxis domain={["auto", "auto"]} fontSize={12} stroke={chartColors.text} />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Line type="monotone" dataKey="weight" stroke={chartColors.primary} strokeWidth={2} dot={{ r: 3 }} name="Weight (kg)" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="chart-container"> 
              <ResponsiveContainer width="100%" height="100%"> 
                <BarChart data={aggregatedData}> 
                  <CartesianGrid strokeDasharray="3 3" stroke={chartColors.grid} /> 
                  <XAxis dataKey="date" fontSize={12} stroke={chartColors.text} /> 
                  <YAxis fontSize={12} stroke={chartColors.text} /> 
                  <Tooltip contentStyle={tooltipStyle} /> 
                  <Legend /> 
                  <Bar dataKey="workoutDays" fill={chartColors.primary} name="Workout Days" /> 
                </BarChart> 
              </ResponsiveContainer> 
            </div> 
          </div>
            ...
            <div className="chart-container-lg">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={aggregatedData}>
                  ...
                  <Bar dataKey="avgBurned" fill={chartColors.danger} name="Avg Burned (kcal)" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
              ...
              <div className="chart-container-sm">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={formattedLogs}>
                    <CartesianGrid strokeDasharray="3 3" stroke={chartColors.grid} />
                    <XAxis dataKey="date" fontSize={10} stroke={chartColors.text} />
                    <YAxis fontSize={10} stroke={chartColors.text} />
                    <Tooltip contentStyle={tooltipStyle} />
                    <Line type="monotone" dataKey="protein" stroke={chartColors.purple} strokeWidth={2} dot={false} name="Protein (g)" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="chart-container-sm">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={formattedLogs}>
                    ...
                    <YAxis fontSize={10} stroke={chartColors.text} />
                    <Tooltip contentStyle={tooltipStyle} />
                    <Line type="monotone" dataKey="water" stroke={chartColors.cyan} strokeWidth={2} dot={false} name="Water (L)" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <div className="card"> 
          <div className="flex-column gap-12"> 
            {bodyComp.insights.map((insight, i) => ( 
              <div key={i} className="text-small"> <strong>{insight.note}</strong> 
              </div> 
            ))} 
          </div> 
      ...
      */} 
      ...
      {review && review.stats && Object.keys(review.stats).length > 0 ? (
        <div className="card">
          <div className="card-grid grid-auto-8">
